...
- **Server/client split**: `app/page.tsx`, `app/contact/page.tsx`, `app/projects/page.tsx`, `app/ask/page.tsx`, `app/darkroom/page.tsx`, `app/vault/page.tsx` are server components that export `metadata`; interactive halves are `ProjectsList.tsx` (sorts rows the server built — the projects data module never enters a client bundle), `AskClient.tsx`, `DarkroomClient.tsx`, `VaultClient.tsx`. `ui.tsx` has no `"use client"` (its primitives are stateless); the `Accordion` lives in `app/components/Accordion.tsx` and is the one primitive that hydrates. Media inside a row goes through `DeferredMedia` so a collapsed row downloads nothing. Keep it that way — a `"use client"` page cannot export metadata.
