...
**The plumbing is the hard part, not the model.** My desktop sits behind NAT and I wasn't willing to forward a port to run a toy. It began strictly one-way: the Vercel route pushed a job onto a hosted Redis queue, the desktop long-polled that queue over HTTPS, ran the question against a local Ollama model, and wrote the answer back under the job's id. Nothing ever reached in.

That held until the queue's free tier ran out mid-month and took the concierge offline — twice, the second time for days. Rationing the polling would only have bought another month, so the queue moved onto the desktop itself and Vercel now talks to it through a Tailscale Funnel. There is still no port forwarded on my router and still a certificate I don't manage, but I'd rather say plainly that the traffic runs inward now instead of outward, because that is a real change and the earlier version of this page claimed otherwise. A bearer token guards the endpoint and a second shared secret is checked inside the job payload, so a leaked URL on its own buys nothing. The desktop publishes a heartbeat with a short TTL — if it goes stale the site knows the node is down and says so, in plain language, with links, instead of hanging.
...
**Built with** Next.js API routes on the site side; a single-file Python poller, a small Redis-dialect relay, Ollama, Tailscale and systemd on the desktop side.
